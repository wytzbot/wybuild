 ...
 @override Widget build(BuildContext c){
  ...
  return shell('WORKSPACE','Projects','Pick a repository. WyBuild will diagnose the project, set up the workflow and run the build.',Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
   ...
   card(Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
    ...
    if(repo!=null) ...[const SizedBox(height:12),DropdownButtonFormField<String>(value:branch.isEmpty?null:branch,decoration:const InputDecoration(labelText:'Branch'),items:branches.map((b)=>DropdownMenuItem(value:b['name'],child:Text(b['name']))).toList(),onChanged:(v){if(v!=null){setState(()=>branch=v);diagnose();}},)],
   ])),
   if(repo!=null) ...[
    ...
    card(Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
      ...
      const SizedBox(height:12),btn(loading?'Building…':'Build Now',loading?null:build,icon:Icons.rocket_launch),
    ])),
   ]
  ]));
 }
