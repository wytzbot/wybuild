...
class _ProjectsState extends State<Projects>{
 ...
 @override void initState(){super.initState();if(widget.session!=null){loadRepos();loadPlan();}}
  Future<void> loadPlan() async {setState(()=>planLoading=true);try{final x=await api.call('/api/billing/status');if(mounted)setState(()=>plan=String(x['plan']??'FREE').toUpperCase());}catch(_){ }finally{if(mounted)setState(()=>planLoading=false);}}
