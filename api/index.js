...
const ANDROID_SHELL_FILES = {
  ...
  "build.gradle": "plugins { id 'com.android.application' version '8.7.3' apply false }\nallprojects { configurations.configureEach { exclude group: 'org.jetbrains.kotlin', module: 'kotlin-stdlib-jdk7'; exclude group: 'org.jetbrains.kotlin', module: 'kotlin-stdlib-jdk8'; resolutionStrategy.eachDependency { d -> if (d.requested.group == 'org.jetbrains.kotlin') d.useVersion '2.4.10' } } }\n",
  "gradle.properties": "android.useAndroidX=true\nandroid.enableJetifier=false\n",
  "app/build.gradle": "plugins { id 'com.android.application' }\nandroid { namespace 'com.wybuild.wrapper'; compileSdk 35; defaultConfig { applicationId 'com.wybuild.wrapper'; minSdk 23; targetSdk 35; versionCode 1; versionName '1.0' } }\ndependencies { implementation platform(\"org.jetbrains.kotlin:kotlin-bom:2.4.10\"); implementation \"org.jetbrains.kotlin:kotlin-stdlib:2.4.10\"; implementation \"androidx.fragment:fragment:1.8.5\" }\n",
  "app/src/main/AndroidManifest.xml": "<manifest xmlns:android=\"http://schemas.android.com/apk/res/android\">\n <uses-permission android:name=\"android.permission.INTERNET\"/>\n <application android:theme=\"@style/AppTheme\" android:label=\"__APP_NAME__\" android:usesCleartextTraffic=\"false\" android:hardwareAccelerated=\"true\" android:supportsRtl=\"true\">\n  <activity android:name=\".MainActivity\" android:exported=\"true\" android:screenOrientation=\"unspecified\">\n   <intent-filter><action android:name=\"android.intent.action.MAIN\"/><category android:name=\"android.intent.category.LAUNCHER\"/></intent-filter>\n  </activity>\n </application>\n</manifest>\n",
  ...
  try {
    ...
    if (req.method === 'POST' && route === 'github/install-workflow') {
      ...
      try {
        ...
        // The workflow's "Prepare maintained WyBuild native shell" step expects
        // .wybuild/android-shell/* to already exist in the checked-out repo (it
        // copies that folder into the generated Android wrapper). Without this,
        // every Web -> APK/AAB build on a target repo fails as soon as it reaches
        // that step's `test -f .wybuild/android-shell/...` checks, since only the
        // WyBuild platform's own repo ever had that folder. Commit the maintained
        // shell alongside the workflow so target repos actually have it.
        for (const [relPath, contents] of Object.entries(ANDROID_SHELL_FILES)) {
          const repoPath = `.wybuild/android-shell/${relPath}`;
          let shellSha;
          try {
            const existingShell = await gh(`/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contents/${repoPath}?ref=${encodeURIComponent(branch)}`, s.token);
            shellSha = existingShell.sha;
          } catch (e) {
            if (e.status !== 404) throw e;
          }
          await gh(`/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contents/${repoPath}`, s.token, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              message: shellSha ? 'chore: update WyBuild native shell' : 'chore: add WyBuild native shell',
              content: Buffer.from(contents).toString('base64'),
              branch,
              ...(shellSha ? { sha: shellSha } : {})
            })
          });
        }
      } catch (e) {
